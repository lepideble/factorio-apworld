...
from .Data import data, FactorioElement, machines, recipes as base_recipes, recipes_mining_with_fluid, recipes_unlocked_at_start
...
for technology_name, technology_data in sorted(data["technology"].items()):
    technology = Technology(
        technology_name,
        factorio_tech_id,
        modifiers=[effect["type"] for effect in technology_data.get("effects", []) if effect["type"] != "unlock-recipe"],
        unlocks=set([effect["recipe"] for effect in technology_data.get("effects", []) if effect["type"] == "unlock-recipe"]) - recipes_unlocked_at_start,
    )
    factorio_tech_id += 1
    technology_table[technology_name] = technology
    if "mining-with-fluid" in technology.modifiers:
        technology.unlocks.update(recipes_mining_with_fluid)
...
ignored_recipes = {
    ...
    "refined-metallic-asteroid-crushing", "advanced-metallic-asteroid-crushing"
}

recipes = {}
all_product_sources: Dict[str, Set[Recipe]] = {"character": set()}

for recipe_name, recipe_data in base_recipes.items():
    recipe = Recipe(
        recipe_name, 
    ...
    if set(recipe.products).isdisjoint(
        ...
        if recipe.name in ignored_recipes or (recipe.name.endswith("-recycling") and recipe.name != "scrap-recycling"):
            continue

        for product_name in recipe.products:
            if product_name == "fluoroketone-hot" and recipe.name != "fluoroketone":
                continue
            if product_name == "stone" and recipe.name != "metallic-asteroid-crushing":
                continue
...
config.override_data(technology_table, recipes, recipes_unlocked_at_start, machines)
...
def recursively_get_unlocking_technologies(ingredient_name, _done=None, unlock_func=unlock_just_tech) -> Set[
    ...
    for recipe in recipes:
        if recipe in recipes_unlocked_at_start:
            continue
...
# Hotfix the logistic-science-pack to represent the fact that we can craft it with the starting items
required_technologies["logistic-science-pack"] = frozenset()
...
fluids: Set[str] = set([fluid_name for fluid_name, fluid_data in data["fluid"].items()])
...
item_stack_sizes: Dict[str, int] = {item_name: item_data["stack_size"] for item_name, item_data in data["item"].items()}
