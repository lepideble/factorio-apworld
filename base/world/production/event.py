...
class FactorioProductionEventItem(Item):
    game = game_name
    production: list[str, ProductionLevel]
    surface: Surface

    def __init__(self, player: int, name: str, production: list[str, ProductionLevel], surface: Surface):
...
def collect_production_event(state, item: FactorioProductionEventItem) -> bool:
    for item_name in item.get_production_item_names():
